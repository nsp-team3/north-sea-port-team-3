...
type bridge = {
	...
	icoo: string,
	icot: string,
	kolkarray: { Width: string, Length: string, Note: string, SchutLengteEb: string, SillDepthBeBu: string, SillDepthBoBi: string }
	lat: number,
	lng: number,
	lnk: string,
	name: string,
...
export default class BridgeLayer extends Layer {
	...
	private async updateNearbyBridges(): Promise<void> {
		...
		if (nearbyBridges) {
			this.clearLayers();
			nearbyBridges.forEach((bridgeData: bridge) => this.displayBridge(bridgeData));
	 ...
	 */
	private displayBridge(bridge: bridge): void {
	 ...
	 */
	private async handleBridgeClick(event: L.LeafletMouseEvent, bridge: bridge): Promise<void> {
	...
	private async parsePhoneNumber(bridge: bridge): Promise<string | void> {
	...
	private createBridgeMarker(bridge: bridge): L.Marker {
		return new L.Marker([
			bridge.lat,
			bridge.lng
		], {
	...
	private createBridgeIcon(bridge: bridge): L.Icon {
		return new L.Icon({
